// Forked from AvalonEdit for UnoEdit — SystemColorHighlightingBrush removed (no WPF SystemColors).
// Original: ICSharpCode.AvalonEdit/Highlighting/Xshd/V2Loader.cs
	...
	static class V2Loader
	{
		...
		static HighlightingBrush ParseColor(IXmlLineInfo lineInfo, string color)
		{
			if (string.IsNullOrEmpty(color))
				return null;
			// System colors are not portable — skip them silently.
		...
		{
			int value;
			return int.TryParse(size, out value) ? value : (int?)null;
		}
		...
		/// <summary>
		/// Returns null for system color references (WPF SystemColors not available in portable builds).
		/// Kept for V1Loader compatibility.
		/// </summary>
		internal static HighlightingBrush GetSystemColorBrush(IXmlLineInfo lineInfo, string name)
		{
			return null; // system colors not available in portable runtime
		}
